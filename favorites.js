...
// had to copy resources here since this loads separately

console.log('favorites.js starting...');
...
console.log('loaded', window.allResources.length, 'resources');
...
function shareResource(resourceName) {
    const resource = window.allResources.find(r => r.name === resourceName);
    if (!resource) return;
    
    const text = `Check out ${resource.name} in the Coppell Community Resource Hub!`;
    if (navigator.share) {
        navigator.share({
            title: resource.name,
            text: text,
...
function loadFavorites() {
    ...
    if (favorites.length === 0) {
        grid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 4rem 2rem;">
                ...
                <h3 style="font-size: 1.5rem; color: #111827; margin-bottom: 0.5rem;">No Saved Resources</h3>
                <p style="color: #6b7280; margin-bottom: 1.5rem;">Start saving resources by clicking the heart icon!</p>
                <a href="website.html" style="padding: 0.75rem 2rem; background: #3b82f6; color: white; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-block;">
                    Browse Resources
                </a>
            </div>
        `;
    ...
    for (let i = 0; i < window.allResources.length; i++) {
        const resource = window.allResources[i];
        if (favorites.includes(resource.name)) {
            favoriteResources.push(resource);
        }
    }
    
    console.log('found', favoriteResources.length, 'matching resources');
    
    // build the HTML
    let html = '';
    for (let i = 0; i < favoriteResources.length; i++) {
        const r = favoriteResources[i];
        const resourceIndex = window.allResources.indexOf(r);
        ...
        <div class="resource-card ${r.category}">
            ...
            <div class="card-body">
                <p style="color: #6b7280; margin-bottom: 1rem;">${r.description}</p>
                <div class="card-actions">
                    <button class="action-btn favorite-btn favorited" 
                    onclick="toggleFavorite('${r.name.replace(/'/g, "\\'")}', this)">
                        <i class="fas fa-heart"></i>
                    </button>
                    <button class="action-btn" 
                    onclick="shareResource('${r.name.replace(/'/g, "\\'")}')">
                        <i class="fas fa-share-alt"></i>
                    </button>
                    <button class="action-btn" 
                    onclick="openResourceModal('${r.name.replace(/'/g, "\\'")}')">
                        <i class="fas fa-info-circle"></i>
                    </button>
                </div>
                <div class="card-details">
                    <div style="color: #374151; font-size: 0.95rem; line-height: 1.7; margin-bottom: 1.25rem;">${r.fullDetails}</div>
                    <div style="background: #f9fafb; padding: 1rem; border-radius: 8px; margin-top: 1rem;">
                        <h4 style="color: #111827; margin-bottom: 0.75rem;"><i class="fas fa-address-book"></i> Contact Information</h4>
                        <div style="display: flex; align-items: start; gap: 0.75rem; padding: 0.5rem 0;">
                            <i class="fas fa-map-marker-alt" style="color: #2563eb; min-width: 18px;"></i>
                            <span>${r.address}</span>
                        </div>
                        <div style="display: flex; align-items: start; gap: 0.75rem; padding: 0.5rem 0;">
                            <i class="fas fa-phone" style="color: #2563eb; min-width: 18px;"></i>
                            <a href="tel:${r.phone.replace(/\D/g, '')}" style="color: #2563eb; text-decoration: none;">${r.phone}</a>
                        </div>
                        <div style="display: flex; align-items: start; gap: 0.75rem; padding: 0.5rem 0;">
                            <i class="fas fa-globe" style="color: #2563eb; min-width: 18px;"></i>
                            <a href="${r.website}" target="_blank" style="color: #2563eb; text-decoration: none;">Visit Website</a>
                        </div>
                    </div>
                </div>
            </div>
...
function downloadFavorites() {
    ...
    let text = 'My Saved Coppell Resources\n';
    text += '=========================\n\n';

    favorites.forEach(favName => {
        const resource = window.allResources.find(r => r.name === favName);
        if (resource) {
            text += `${resource.name}\n`;
            text += `Category: ${getCategoryName(resource.category)}\n`;
    ...
    a.href = url;
    a.download = 'coppell-favorites.txt';
    a.click();
...
function clearAllFavorites() {
    if (confirm('Are you sure you want to clear all saved resources?')) {
